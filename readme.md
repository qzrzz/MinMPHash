...
`MinMPHash` 可以把一组数量 n 的字符串映射到 `[0, n-1]` 的整数范围内，且不会有冲突

> 发现 JavaScript/TypeScript 平台上缺乏能用的最小完美哈希实现，于是自己动手写了一个 CHD 的简单实现
> 没有特别严谨，但是能用 😄

`MinMPLookup` 则是在 `MinMPHash` 的基础上实现的最小完美查找表工具

它可以把 `{ key1: [value1, value2, ...], key2: [value3, value4, ...] }` 的这样的映射表最小化存储，实现根据 `value` 查找对应的 `key` 的需求。

相比原样存储，最小化的查找表可以将体积缩小到原来的 10% 以下（根据数据集（values）的信息熵决定，values 熵越大，压缩效果越好）

## 什么是最小完美哈希？

如果你知道哈希函数，那你肯地能理解哈希函数可以把数据映射到一个范围内，也就是生成固定长度的“指纹”，但是普通哈希函数有 2 个问题：

- 稀疏性导致的空间浪费：哈希值域通常远大于实际数据量，哈希表空间极其稀疏。
- 不同输入可能碰撞：不同的输入可能会映射到相同的哈希值，导致冲突 ，为了避免减小冲突的可能，需要更长的哈希值，进一步浪费空间

而最小完美哈希函数则是一个特殊的哈希函数，它能确保没有冲突，并且输出范围正好等于输入数据的数量
...
一个映射表（Map），但是一个普通的映射表（Map）需要存储完整的键（Key）和值（Value），而使用最小完美哈希相当于不需要存储完整的值（Value），只需要存储值（Value）的哈希值，就可以等价地实现键值映射。

最好不过的是，只要值（Value）是确定性的（在数据集内），无论多长都可以用一个 `number`（根据数据集数量）来存储，并且是唯一的不会有冲突。

这是普通哈希函数（MD5、SHA-1 等）无法做到的，并且它们非常长，比如 MD5 要 16 字节，SHA-1 要 20 字节，而最小完美哈希只需要根据数据集的数量来决定存储多大的整数。
...
但使用最小完美哈希，我们可以只存储字体名称的哈希值，而不是完整的名称：
...
这样我们可以大大减少存储空间，因为我们不需要存储完整的字体名称，只需要存储哈希值。

> 你可能会想到 MD5 或 SHA-1 之类的哈希函数，也可以做到类似的效果，但它们生成的哈希值非常长（16 字节或 20 字节）  
> FNV-1a 可以最小 4 字节，但此时冲突率会非常高，而最小完美哈希可以根据数据集的大小动态调整哈希值的长度，并且始终是极限值，没有空间浪费和冲突
...
const dict = createMinMPHashDict(keys, {
  ...
  // 校验强度:
  // "none" - 不校验 (体积最小，有误报)
  // "4"    - 4-bit 指纹 (体积增加 0.5 byte/key，误报率 ~1/16)
  // "8"    - 8-bit 指纹 (体积增加 1 byte/key，误报率 ~1/256) [默认]
  // "16"   - 16-bit 指纹 (体积增加 2 bytes/key，误报率 ~1/65536)
  // "32"   - 32-bit 指纹 (极低误报率)
  onlySet: "8",
});
```
