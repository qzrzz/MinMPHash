import { createMinMPLookupDict, MinMPLookup, IValidationMode } from "../src/index";
...
console.log(`Generating dataset of size ${c.green}${datasetSize}${c.reset} values...`);
...
for (let i = 0; i < keyCount; i++) {
    ...
    for (let j = 0; j < valuesPerKey; j++) {
        ...
        const val = `Value-${valIdx}-${Math.random().toString(36).substring(7)}`;
        values.push(val);
        allValues.push(val);
    }
...
console.log(
  `${c.bold}Dataset stats:${c.reset} ${c.green}${Object.keys(lookupMap).length}${c.reset} keys, ${c.green}${allValues.length}${c.reset} values\n`
);
  ...
  for (const onlySet of onlySetOptions) {
    try {
        ...
        if (allValues.length > 0) {
            ...
            if (!lookup.query(firstVal)) throw new Error(`Validation Failed: ${firstVal} not found`);
            if (!lookup.query(lastVal)) throw new Error(`Validation Failed: ${lastVal} not found`);
        }
        ...
        console.log(`  │ ${onlySet.padEnd(9)}│ ${c.red}Error: ${(e as Error).message.substring(0, 50)}...${c.reset}`);
    }
  }
  console.log(
    `${c.dim}  └──────────┴──────────────┴──────────────────────┴──────────────────────┴──────────────┴──────────────┘${c.reset}\n`
