...
export class MinMPHFilter {
  ...
  constructor(dictOrData: IMinMPHFilterDict | Uint8Array) {
    ...
    this.m = BigInt(this.n) * (1n << BigInt(this.k));
  }
  ...
  has(key: string): boolean {
    ...
    if (this.checkpoints) {
      ...
      if (bestIdx !== -1) {
        ...
        const isLastCheckpoint = bestIdx === this.checkpoints.hashes.length - 1;
        if (isLastCheckpoint) {
          elementsToScan = this.n - (bestIdx * this.checkpoints.interval);
        } else {
          elementsToScan = this.checkpoints.interval;
        }
      }
    }

    if (bestIdx > 0 && startHash === h) return true;
...
function dictToBinary(dict: IMinMPHFilterDict): Uint8Array {
  ...
  // Header
  writeVarInt(dict.n, headerParts);
  writeVarInt(dict.k, headerParts);
  writeVarInt(dict.data.length, headerParts);
  
  const checkpointParts: number[] = [];
  // Checkpoints
  if (dict.checkpoints) {
    writeVarInt(1, checkpointParts); // Has checkpoints
    writeVarInt(dict.checkpoints.interval, checkpointParts);
    writeVarInt(dict.checkpoints.hashes.length, checkpointParts);
    
    // Delta encode hashes (using BigInt)
    let lastH = 0n;
    for (let i = 0; i < dict.checkpoints.hashes.length; i++) {
      const h = dict.checkpoints.hashes[i];
      writeVarBigInt(h - lastH, checkpointParts);
      lastH = h;
    }
    ...
    for (let i = 0; i < dict.checkpoints.offsets.length; i++) {
      ...
      writeVarInt(off - lastOff, checkpointParts);
      lastOff = off;
    }
  } else {
    writeVarInt(0, checkpointParts); // No checkpoints
  }
  
  // Combine: Header + Data + Checkpoints
  const totalLength = headerParts.length + dict.data.length + checkpointParts.length;
  const result = new Uint8Array(totalLength);
  
  result.set(headerParts, 0);
  result.set(dict.data, headerParts.length);
  result.set(checkpointParts, headerParts.length + dict.data.length);
...
function dictFromBinary(bin: Uint8Array): IMinMPHFilterDict {
  ...
  // Use subarray to avoid copying memory (Zero-copy)
  const data = bin.subarray(offset, offset + dataLen);
  offset += dataLen;
