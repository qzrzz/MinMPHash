import { dictFromCBOR, dictToCBOR } from "./util"
...
export function createMinMPHashDict(
    dataSet: string[],
    options?: {
        onlySet?: boolean | ValidationMode
        level?: number
        outputBinary?: false
    }
): IMinMPHashDict
    ...
    for (let i = 0; i < m; i++) {
        ...
        while (!found) {
            ...
            if (!collision) {
                ...
                s++
                if (s > MAX_TRIALS) {
                    throw new Error(`MPHF Failed: Bucket ${i} (size ${k}) is too hard to hash.`)
                }
            }
        }
    }
    ...
    constructor(dict: IMinMPHashDict | Uint8Array) {
        ...
        if (this.validationMode !== "none" && dict.fingerprints) {
            ...
            if (this.validationMode === "2" || this.validationMode === "4" || this.validationMode === "8") {
                ...
                this.fingerprints = raw instanceof Uint32Array ? raw : new Uint32Array(raw as number[])
            }
        }
    }
